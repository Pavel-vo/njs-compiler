...
(function () {
    ...
    // Pavel Nikotin fix, change Array.prototype is bad idea
    // top now is njsfixes.top
            ...
            case TRY:
                ...
                while (t.match(CATCH)) {
                    ...
                    if (t.match(IF)) {
                        ...
                        if (n.catchClauses.length && !njsfixes.top(n.catchClauses).guard)
            ...
            while ((tt = t.get()) != END) {
                ...
                switch (tt) {
                    ...
                    case COLON:
                        ...
                        while (opPrecedence[njsfixes.top(operators).type] > opPrecedence[tt])
                            reduce();
                        if (tt == COLON) {
                            n = njsfixes.top(operators);
                            if (n.type != HOOK)
                            ...
                            if (tt == ASSIGN)
                                njsfixes.top(operands).assignOp = t.token().assignOp;
                            else
                                ++x.hookLevel;      // tt == HOOK
                        }
                        t.scanOperand = true;
                    ...
                    case YIELDING:
                        while (opPrecedence[njsfixes.top(operators).type] > opPrecedence[YIELDING])
                            reduce();
                        t.mustMatch(LEFT_PAREN);
                        var yielding = true;
                    // FALL THROUGH
                    ...
                    case LEFT_PAREN:
                        if (t.scanOperand) {
                            ...
                            while (opPrecedence[njsfixes.top(operators).type] > opPrecedence[NEW])
                                reduce();
...
var NJS_CODE = Narcissus.REGEXP; // fake node type for njs code segment
...
njscp.compileOrig = function (/*string*/ code, /*string*/ scriptName) {
...
njscp.compile = function (text) {
    return eval(this.compileOrig('var ___f___=' + text + ';\n ___f___;')).toString();
};

njscp.compilef = function (func) {
    return eval(this.compileOrig('var ___f___=' + func.toString() + ';\n ___f___;'));
};
        ...
        switch (node.type) {
            ...
            case FUNCTION:
                ...
                if (isYielding(node.body)) {
                    ...
                    var openNodes = [
                        codeNode('var ~=' + options.runtime + '.njen(this,arguments~);',
            ...
            case CALL:
                ...
                if (node[0].type == DOT) {
                    ...
                    caller = codeNode(options.runtime + '.NJNEW');
                    callee = node[0];
                } else {
                ...
                var checkRetNode = noderize({
                    ...
                    body: noderize({
                        ...
                        condition: noderize({
                            ...
                            1: codeNode(options.runtime + '.NJSUS')
                        }),
                        thenPart: noderize({
                            type: SEMICOLON,
            ...
            case FOR_IN: // varDecl/iterator, object, body
                ...
                statements.push(noderize({
                    type: NJS_CODE,
                    value: subst("~.iter~=" + options.runtime + ".njkeys(", scopeId, iterId),
                ...
                for (var i = 0; i < node.catchClauses.length; i++) {
                    ...
                    if (clause.guard) {
                        ...
                        if (i < node.catchClauses.length - 1) {
                            ...
                            gotoCodeSegment(njsfixes.top(exPtrStack));
                        } else {
...
nsrp.addSymbol = function (name) {
    njsfixes.top(this.scopes)['_' + name] = true;
...
nsrp.isYielding = function () {
    ...
    return njsfixes.top(this.yieldingStatus);
}
...
njsswp.last = function () {
    return njsfixes.top(njsfixes.top(this.lines));
}

njsswp.pop = function () {
    return njsfixes.top(this.lines).pop();
}
...
module.exports.create = function (options) {
    return new NjsCompiler(options);
};
